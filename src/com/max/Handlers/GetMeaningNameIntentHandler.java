...
import com.max.Utils.GoogleWebSearch;
import com.max.Utils.SearchQuery;
import com.max.Utils.SearchResult;
...
public class GetMeaningNameIntentHandler implements RequestHandler
{
    ...
    private Optional<Response> setSuccessResultForName(HandlerInput handlerInput,Session session,String meaningName, String meaning)
    {
        if (meaningName != null)
        {
            ...
            String speechText = "Definition of " + meaningName + " Would you like to get the definition for another name. Ask to alexa, definition for \"some word\".";
            ...
            return handlerInput.getResponseBuilder().withSpeech(speechText).withSimpleCard(cardTitle,speechText).withReprompt(rePromptText).build();
        }
        else
        {
            String speechText = "I could not find any name to repeat the definition. Please, say a new name to get the definition. Please, say the valid name to get the definition for that name.";
    ...
    private Optional<Response> sendFailureResult(HandlerInput handlerInput)
    {
        ...
        String rePrompt = "Please, say name again.";

        return handlerInput.getResponseBuilder().withSpeech(speechText).withReprompt(rePrompt).build();
    }
}
